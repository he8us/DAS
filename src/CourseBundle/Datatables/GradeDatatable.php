...
class GradeDatatable extends AbstractDatatableView
{
    ...
    public function buildDatatable(array $options = [])
    {
        ...
        $this->ajax->set([
            'url'      => $this->router->generate('course_grade_results'),
            'type'     => 'GET',
            'pipeline' => 0,
        ]);
        ...
        $this->columnBuilder
            ->add('id', 'column', [
                'title' => $this->translator->trans('course.grade.id'),
            ])
            ->add('grade', 'column', [
                'title' => $this->translator->trans('course.grade.grade'),
            ])
            ->add(null, 'action', [
                'title'   => $this->translator->trans('course.datatable.actions.title'),
                'actions' => [
                    [
                        'route'            => 'course_grade_show',
                        'route_parameters' => [
                            'id' => 'id',
                        ],
    ...
    {
        return 'CourseBundle\Entity\Grade';
    }
